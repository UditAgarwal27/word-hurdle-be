...
router.post('/storeData', async(req, res)=>{
    ...
    const access_token = (access_token_withBearer === null || access_token_withBearer === undefined) ? "" : access_token_withBearer.split(" ")[1];

    if(!access_token_withBearer) return res.status(400).json({msg:"User did not send access token"});
    ...
    .then(async(response)=>{
        if(!response) return res.status(401).json({msg:"User is not authorized"})

        const emailFromGoogle = response.data.emailAddresses[0].value;
        if(email !== emailFromGoogle) return res.status(401).json({msg:"Unauthorized! Invalid access token"});

        await leaderboardModel.findOne({"email":email})
        .then(async(leaderboardResponse)=>{
            ...
            await leaderboardModel.findOneAndUpdate({email:email},{score: score} )
...
router.get('/getAllUsersData', async(req, res)=>{
    ...
    .then(response=>{
        if(!response) return res.status(500).json({msg:"Server error in fetching the leaderboard records"});
    ...
    .catch(err=>{
        console.log("Server error in fetching the leaderboard score of every user");
    })
})
